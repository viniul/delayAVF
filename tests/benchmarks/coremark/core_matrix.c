...
void main(void){
    ...
    result.seed2 = 0x3409;
    result.seed3 = 0x66;
    result.memblock[0] = (void *)static_memblk;
    result.size        = TOTAL_DATA_SIZE;
    result.err         = 0;
    result.crc = 0;
    result.seed3 = crcu16(result.seed3, 0x00);
    core_init_matrix(result.size, result.memblock[0], (ee_s32)result.seed1 | (((ee_s32)result.seed2) << 16), &(result.mat));
    retval = core_bench_matrix(&(result.mat), result.seed3, result.crc);
    if (result.crcmatrix == 0){
        result.crcmatrix = retval;
    }
    result.crc = crcu16(retval, result.crc);
    ee_printf("CRC: %x \n", result.crc); //Take timeout=1801000 on pico to reach
}