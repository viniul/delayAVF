...
void main(void){
    ...
    result.err         = 0;
    crc = 0;
    core_init_state(result.size, result.seed1, result.memblock[0]);
    //results.list = core_list_init(results.size, results.memblock[1], results.seed1);

    crc = core_bench_state(result.size, result.memblock[0], result.seed1, result.seed2, 0x22, crc);
    ee_printf("CRC: %x \n", crc); //Take timeout=1801000 on pico to reach
}