...
cmp_idx(list_data *a, list_data *b, core_results *res)
{
    if (res == NULL)
    {
        //[11/29/2023 VU]: I commented out the below code. Why does the data change here???
...
core_list_init(ee_u32 blksize, list_head *memblock, ee_s16 seed)
{
    ...
    //ee_printf("Memblock end %p size %d memblock %p size %d:\n", memblock_end, size, memblock, sizeof(list_data));
    ...
    list->info->data16 = (ee_s16)1;//0x8080;
    memblock++;
    datablock++;
    info.idx    = 0x7fff;
    info.data16 = (ee_s16)2;//0xffff;
    //ee_printf("Initializing new elements:\n");
    core_list_insert_new(
        list, &info, &memblock, &datablock, memblock_end, datablock_end);
    //ee_printf("Called list->next %p info %p:\n", list->next, info);
    ...
    for (i = 0; i < size; i++)
    {
        ...
        info.data16 = 32; //(dat << 8) | dat;  /* fill the data with actual data and
                          //                  upper bits with rebuild value */
        core_list_insert_new(
            list, &info, &memblock, &datablock, memblock_end, datablock_end);
    }
    //ee_printf("Size items done       \n");
    /* and now index the list so we know initial seed order of the list */
    finder = list->next;
    //ee_printf("Called list->next %p:\n", finder);
    i      = 1;
    ...
    //ee_printf("Initialized list pre-sort:\n");
    //finder = list;
    //while (finder)
    //{
    //    ee_printf(
    //        "%p [%04x,%04x] %d \n", finder, finder->info->idx, (ee_u16)finder->info->data16, finder->info->data16);
    //    finder = finder->next;
    //}
    //ee_printf("\n");
    // ee_printf("Mergesorting the list:\n");
    list = core_list_mergesort(list, cmp_idx, NULL);
#if CORE_DEBUG
                     ...
                     list_data * datablock_end)
{
    ...
    //ee_printf("Setting %p to %p  data %d \n", insert_point->next, newitem, newitem->info->data16);
...
void main(void){
    ...
    results.seed1 = 0x3200;
    results.seed2 = 0xABCDE;
    results.seed3 = 10;
    results.memblock[0] = (void *)static_memblk;
    results.size        = TOTAL_DATA_SIZE;
    results.err         = 0;
    results.list = core_list_init(results.size, results.memblock[0], results.seed2);
    crc = core_bench_list(&results, 1);
    ee_printf("Stop CRC %u \n", crc);           
}