...
void Multiply(matrix A, matrix B, matrix Res)
{
  ...
  ee_printf("Res[0][0] %ld %ld %ld dim %ld \n", Res[0][0], A[0][0], B[0][0], UPPERLIMIT);
}
...
int verify_benchmark(int unused)
{
  ...
  matrix exp = {
    18746573, 32211547, 30901039,     
14221566,     25524528,   37630429,     
46614392,    100403290,     111395864,  
  };
#endif
  //for (int row=0; row<UPPERLIMIT; row++)
  //{
  //    for(int columns=0; columns<UPPERLIMIT; columns++)
  //    {
  //        ee_printf("%ld     ", ResultArray[row][columns]);
  //    }
  //    ee_printf("\n");
  //}
  ...
  for (i=0; i<UPPERLIMIT; i++){
    for (j=0; j<UPPERLIMIT; j++){
      if (ResultArray[i][j] != exp[i][j]){
        //ee_printf("NC %d %d %d %d!\n", ArrayA[i][j], ArrayB[i][j], ResultArray[i][j], exp[i][j]);
        //ee_printf("NC %d %d %d %d!\n", ArrayA[i][j], ArrayB[i][j], ResultArray[i][j], exp[i][j]);
        ee_printf("NC \n");
        return 0;
      }
